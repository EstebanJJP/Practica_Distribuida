...
client = mqtt.Client("Mesa/contemporanea")
...
def philosopher_task(num:int, table: Table):
    ...
    while True:
        client.publish("Mesa/contemporanea", str(Phil_names[num])+" esta pensando")
        print (f"{Phil_names[num]} esta pensando")
        delay(6)
        client.publish("Mesa/contemporanea", str(Phil_names[num])+" quiere comer")
        print (f"{Phil_names[num]} quiere comer")
        table.wants_eat(num)
        client.publish("Mesa/contemporanea", str(Phil_names[num])+" esta comiendo")
        print (f"{Phil_names[num]} esta comiendo")
        table.wants_think(num)
        delay(6)
        client.publish("Mesa/contemporanea", str(Phil_names[num])+" para de comer")
