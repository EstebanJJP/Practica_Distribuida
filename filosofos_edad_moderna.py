...
NPHIL = 5
Phil_names = ["Descartes", "Locke", "Spinoza", "Rousseau", "Voltaire"]
K = 100
...
def philosopher_task(num:int, table: Table):
    ...
    while True:
        client.publish("Mesa_edad_moderna", str(Phil_names[num])+" estÃ¡ pensando")
        ...
        table.wants_eat(num)
        client.publish("Mesa_edad_moderna", str(Phil_names[num])+" estÃ¡ comiendo")
